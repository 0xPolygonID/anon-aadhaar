...
async function prepareTestData() {
  ...
  const updateTemplate = [
    ...
    "13483382060079230067188057675928039600565406666878111320562435194759310415773", "1766523189512228532", // expirationDate
  ...
  const inputs = {
    ...
    expirationDate: "1766523189512228532",
...
describe('AadhaarVerifier', function () {
  ...
  it('should output extracted data if reveal is true', async () => {
    ...
    // verify that the schema processor and the circuit produce the same final claim's root
    assert(witness[3] === BigInt("15707964129774932695113360264469755583670183541687363022317951487056569865555"));
    // compare the results with the results of the go-core library
    assert(witness[4] === BigInt("19596681573519173113304247698414935467125259772085846841368639201700390543674"));
    assert(witness[5] === BigInt("5101116276114669200802560758548423593300623108682712978636844678294687470999"));
  })
})
